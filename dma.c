...
void DMA_config(uint16_t srcAddr[],uint16_t nbytes, DMA_mode_t transfer_mode)
{
    ...
    if(DMA_ContinuosTransfer == transfer_mode){

        EDMA_TcdSetMajorOffsetConfig(
    		    		   tcdMemoryPoolPtr, 							/* param tcd A point to the TCD structure. */
						   -nbytes, 											/* param sourceOffset source address offset*/
    					   0);											/* destOffset destination address offset */
        EDMA_TcdEnableAutoStopRequest(&tcdMemoryPoolPtr[0], false); 	/* false: se hacen múltiples transferencias */

    }
    else if(DMA_OneTransfer == transfer_mode){
        EDMA_TcdSetMajorOffsetConfig(
    		    		   tcdMemoryPoolPtr, 							/* param tcd A point to the TCD structure. */
